...
def generate_launch_description():
    ...
    # ------------------------------------------------------------------
    # 初始构型默认值（左右一致）：
    #   LEFT_INIT 为左臂关节角，RIGHT_INIT 当前 == LEFT_INIT（一致构型）。
    # 若要切换到“不一致”组（如 exp03/06），把 RIGHT_INIT 改成不同的
    # 右臂关节角即可（可同时改注释里的值）。注意：命令行传
    # initial_positions_1/2 实测并不会改变启动构型，只能改这里的默认值。
    # ------------------------------------------------------------------
    LEFT_INIT = '"0.0 -0.785 0.0 -2.356 0.0 1.571 0.785"'   # 左臂：一致构型
    RIGHT_INIT = LEFT_INIT                                   # 右臂：与左臂相同
    # 不一致组（右臂区别于左臂）示例值：
    #     LEFT_INIT = '"0.0 -0.785 0.0 -2.356 0.0 1.571 0.785"'
    #     RIGHT_INIT = '"0.0 -1.100 0.0 -2.800 0.0 1.900 0.900"'
...
"""
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            initial_positions_1_param, default_value=LEFT_INIT, description='Init pos robot 1.'),
        DeclareLaunchArgument(
            initial_positions_2_param, default_value=RIGHT_INIT, description='Init pos robot 2.'),
