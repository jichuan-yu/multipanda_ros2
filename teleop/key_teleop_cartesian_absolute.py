...
This script anchors the command base ONCE to the real end-effector pose computed
by FK from the first /joint_states message (or the first /ee_pose message if that
topic is flowing) at startup (NOT a hardcoded home), then sends absolute
TASK_SPACE_POSE commands (command_type=5) built from that anchored base + key
deltas. Because the anchor is the real robot pose, the same script works for
every initial configuration (symmetric or asymmetric, e.g. exp03/exp06) without
per-experiment changes, and the first command produces zero initial jump.
...
Subscribes:
  - /joint_states: FK-based anchor of the command base to the startup real EE
    (primary source; same DH FK as the relative-error monitor)
  - /ee_pose: actual EE pose feedback (display; also anchors if it is flowing)
...
# FK model (same DH as the controller / relative-error monitor) used to anchor the
# command base to the real startup EE from /joint_states. This is the PRIMARY
# anchor source: /joint_states is always published by the sim, and it reproduces
# the monitor's own initial-relative baseline FK exactly. /ee_pose (if flowing)
# remains available as an alternative display/anchor source.
import sensor_msgs.msg
from rclpy.qos import QoSProfile, ReliabilityPolicy, HistoryPolicy

try:
    from teleop.utils.panda_dh_kinematics import PandaDHKinematics
except ImportError:
    from utils.panda_dh_kinematics import PandaDHKinematics
...
class KeyCartesianAbsoluteTeleop(BaseTeleopNode):
    ...
    The command base is anchored ONCE to the real startup end-effector pose computed
    by FK from the first /joint_states message (falling back to the first /ee_pose
    message if that topic is flowing), never to a hardcoded home. Subsequent
    keypresses add deltas to that anchored base, so commanded relative geometry
    always matches the real initial configuration (symmetric and asymmetric setups)
    and the first command has no jump.
    """
    ...
    def __init__(self, step_position: float = 0.001, step_rotation: float = 0.01):
        ...
        # Current joint positions -> FK anchor (mirrors the monitor's own baseline FK)
        self.q_left = np.zeros(7)
        self.q_right = np.zeros(7)
        self._joint_states_ready = False
        try:
        ...
        # Only used for display; the command base anchor does NOT depend on it.
        ...
        # Subscriber for current joint states -> FK-based absolute anchor
        # (best-effort, same profile as the relative-error monitor)
        js_qos = QoSProfile(
        ...
        self.print_usage()
        # Command base (what key deltas are added to). None until anchored to the
        # real startup EE pose (see _anchor_from_fk / ee_pose_callback).
        # Movement keys are ignored while it is None. Replaces the old hardcoded
        # home so the same script works for symmetric & asymmetric configurations.
...
(Command base anchored ONCE to startup real EE)
========================================
Position Control:
...
Note: Command base is anchored ONCE to the real startup EE pose
      (FK from /joint_states, or /ee_pose if flowing). Wait for
      'Ready for control!' before moving - movement keys are ignored
      until the anchor is set (protects against jumps from a
      hardcoded home, e.g. asymmetric setups exp03/exp06).
...
"""
        ...
        try:
            if len(msg.data) >= 14:
                ...
                if not self.ee_pose_received:
                    ...
                    # Anchor the command base to the real startup EE pose (once).
                    self._lock_base(
                        left_pos.copy(),
                        left_quat.copy(),
                        right_pos.copy(),
    ...
    def _lock_base(self, left_pos, left_quat, right_pos, right_quat, source):
        """One-shot: lock the command base to the real startup EE pose.

        Subsequent keypresses build absolute commands from this base + delta, so
        the commanded relative geometry always matches the real initial config
        (both symmetric and asymmetric setups) and the first command has zero jump.
        """
        if self._anchored:
            return
        self.target_ee_poses = {
            'left': {'position': left_pos, 'quaternion': left_quat},
            'right': {'position': right_pos, 'quaternion': right_quat},
        }
        self._anchored = True
        print(f"\nEnd-effector pose feedback received. Ready for control!", flush=True)
        print(
            f"  Command base anchored to startup real EE pose (source={source}):\n"
            f"  L pos=[{left_pos[0]:7.4f}, {left_pos[1]:7.4f}, {left_pos[2]:7.4f}]\n"
            f"  R pos=[{right_pos[0]:7.4f}, {right_pos[1]:7.4f}, {right_pos[2]:7.4f}]\n",
            flush=True
        )

    def joint_state_cb(self, msg):
        """Read /joint_states and anchor the command base from FK (same DH as the
        controller/monitor). Acts as the primary anchor mechanism so the keyboard
        teleop works even when /ee_pose is not flowing."""
        if not self._have_fk:
            return
        try:
            ...
            if not self._joint_states_ready:
                self._joint_states_ready = True
            self._anchor_from_fk()
        except Exception:
            pass

    def _anchor_from_fk(self):
        """One-shot anchor using the same DH FK as the relative-error monitor."""
        if self._anchored or not self._joint_states_ready:
            return
        try:
            ...
            self._lock_base(
                np.asarray(p_l).reshape(3).astype(float).copy(),
                np.asarray(q_l_wxyz).reshape(4).astype(float).copy(),
                np.asarray(p_r).reshape(3).astype(float).copy(),
                np.asarray(q_r_wxyz).reshape(4).astype(float).copy(),
                source='FK(/joint_states)'
            )
        except Exception:
            pass
    ...
    def update_from_key(self, key: str) -> bool:
        """Update pose based on key input, using anchored startup EE pose as base."""
        # Arm selection
        if key == 'z':
        ...
        # Do not send any movement until the command base is anchored to the
        # real startup EE pose (via FK from /joint_states, or /ee_pose if flowing).
        # Anchoring also means the very first command is the actual current pose
        # -> zero initial jump (critical for asymmetric setups where real EE
        # != any hardcoded home).
        if self.target_ee_poses is None:
            print("\nWaiting for real EE feedback (FK from /joint_states, or /ee_pose)... "
                  "command base will auto-anchor; move keys are ignored until then.\n",
                  flush=True)
            return False
        ...
        if updated:
            # Base = anchored command pose (real startup EE) + key delta
            if self.selected_arm in ['left', 'both']:
                # Base = anchored command pose + delta
